...
if (username.length === 0 || password.length === 0) {
  console.log("Please set username and password");
  return -1;
}
...
_this = {
  ...
  portalInfo: {
    ...
    noticeType: "list",
    nowType: "ipv4",
    selfServiceIp: "http://nap.cug.edu.cn",
    userDevice: {
      device: "Linux",
      platform: "Linux",
    }
...
async function getToken() {
  url = `http://nap.cug.edu.cn${APIs.token}`;
  let ret = await axios
    .get(url, {
      url: APIs.token,
      params: {
...
async function login(obj) {
  ...
  let _login = async function (token) {
    ...
    const url = `http://nap.cug.edu.cn${APIs.auth}`;
